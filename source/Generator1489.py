...
def generate() -> str:
    ...
    n = max_length
    test = []
    skeleton = [[i, i + 1, 100] for i in range(n - 1)]
    for i in range(n - 1):
        if i % 5 == 0:
            w = random.randint(1, 3)
            test.extend([[i, j, w] for j in range(i + 2, n) if j % 2 == 0])
    test = [
        *random.sample(test, min(len(test), 200 - len(skeleton))),
        *skeleton]
    tests.append(str(n) + "\n" + test.__str__())
    ...
    for i in range(n - 1):
        if i % 4 == 0:
            ...
            test.extend([[i, j, w] for j in range(i + 2, n) if j % 5 == 0])
    test = [
        *random.sample(test, min(len(test), 200 - len(skeleton))),
        *skeleton]    tests.append(str(n) + "\n" + test.__str__())
    tests[-1] = tests[-1].replace(" ", "")
    ...
    for i in range(n - 1):
        if i % 5 == 0:
            w = random.randint(1, 5)
            test.extend([[i, j, w] for j in range(i + 2, n) if j % (i + 3) == 0])
    test = [
        *random.sample(test, min(len(test), 200 - len(skeleton))),
        *skeleton]    tests.append(str(n) + "\n" + test.__str__())
    tests[-1] = tests[-1].replace(" ", "")
    ...
    for i in range(n - 1):
        w = random.randint(1, 5)
        test.extend([[i, j, w] for j in range(i + 2, n) if random.randint(0, 1) == 0])
    test = [
        *random.sample(test, min(len(test), 200 - len(skeleton))),
        *skeleton]
    tests.append(str(n) + "\n" + test.__str__())
    ...
    for i in range(n - 1):
        w = random.randint(1, 5)
        test.extend([[i, j, w] for j in range(i + 1, n)])
    tests.append(str(n) + "\n" + test.__str__())
    tests[-1] = tests[-1].replace(" ", "")
    ...
    test = []
    for i in range(n - 1):
        test.extend([[i, j, 10] for j in range(i + 1, n)])
    tests.append(str(n) + "\n" + test.__str__())
    tests[-1] = tests[-1].replace(" ", "")
